use crate::galaxy_hierarchy::{Controllable, DynamicInterceptorMagazineSubsystem, SubsystemBase};
use crate::{FlattiverseEvent, SubsystemSlot, SubsystemStatus};
use std::sync::Weak;

/// Static interceptor magazine subsystem of a modern ship.
#[derive(Debug)]
pub struct StaticInterceptorMagazineSubsystem {
    base: DynamicInterceptorMagazineSubsystem,
}

impl StaticInterceptorMagazineSubsystem {
    pub(crate) fn new(
        ...
        Self {
            base: DynamicInterceptorMagazineSubsystem::new(controllable, name, exists, slot),
        }
    }
...
impl AsRef<SubsystemBase> for StaticInterceptorMagazineSubsystem {
    #[inline]
