...
pub fn hash_password(salt: &str, password: &str) -> [u8; 16] {
    let s_data = sha512(salt);
    let p_data = sha512(password);

    let mut slowness = [0u8; SLOWNESS_LEN];
    ...
    let aes = Aes128::new(&GenericArray::from_slice(&p_data[0..32]));
    let mut iv = [0u8; 16];
    iv.copy_from_slice(&s_data[32..32 + 16]);
    aes.encrypt_block(&mut GenericArray::from_mut_slice(&mut iv)); // init value???

    for _ in 0..7 {
        aes.encrypt_block(&mut GenericArray::from_mut_slice(&mut slowness[..]));
    }
